...
An **A2A client**, not a server — plain TypeScript that composes the three agents. But it is **not a fixed pipeline**: it is an *intelligent content orchestrator* that, given an intent and the current state of the content, decides which capabilities to invoke. Two faces:
...
### Intelligent routing (default mode)

The headline closed loop — *generate → migrate → evaluate* — is **one route, not the only one**. The orchestrator runs a small agentic planner (a Claude Agent SDK `query()` with the three Agent Cards as available tools) that inspects the request and the current state, then chooses the stages:

| Starting state / intent | Route the orchestrator takes |
|---|---|
...
- **Any subset, any order.** Generate, migrate, and evaluate are independently-addressable capabilities; the orchestrator calls only the ones the goal requires.
...
| **M3 — Closed loop 1x + intelligent routing** | mid-August | Content-gen agent (both skills, `sdk` backend). Migration facade with `sdk` backend; `makecom` backend callback wired. One full synthesize → migrate → eval chain via orchestrator, **plus the agentic planner** that skips stages by state (source supplied → migrate→eval; already migrated → eval only; brief-only → stop). | Part 4 + Part 5 definitions of done; orchestrator demonstrates ≥3 distinct routes incl. a non-eval-terminating one |
