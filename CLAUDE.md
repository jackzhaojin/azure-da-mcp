...
> **Version lines**: **v2.x** (currently `v2.9.0`) is the **`agents/` A2A platform** (project #7 — the current flagship workstream). **v1.x** (`v1.1.0`) is the legacy `content-authoring-eval` app — a **frozen backup**, never modified (decision D5). When in doubt about "the new platform" vs "the eval app", that's the v2.x vs v1.x split. Full history: [CHANGELOG.md](./CHANGELOG.md).
...
### Current State
- **Current version**: `v2.9.0` (agents platform, the active line) · `v1.1.0` = the frozen legacy line
...
**Version lines**: **v2.x** = the `agents/` A2A platform (flagship, **v2.9.0 deployed on Cloudflare**; `v2.x+` tags trigger `deploy-agents.yml`) · **v1.1.0** = legacy `content-authoring-eval` (**deprecated**, frozen backup). `agent-claude-sdk/` is also **deprecated**. Trunk-based, tag from `main`; history in [`CHANGELOG.md`](./CHANGELOG.md).
