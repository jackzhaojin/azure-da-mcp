#!/usr/bin/env node
/**
 * Azure Deployment E2E Test for ClaudeLlmClient
 * Tests the Claude LLM endpoint deployed on Azure with a simple haiku request
 *
 * Run with: node tests/azure/claude-haiku-azure.test.js
 */
...
console.log('🚀 Testing ClaudeLlmClient on Azure with Simple Haiku Request');
...
async function testClaudeHaiku() {
  try {
    console.log('📝 Requesting haiku from Claude on Azure...');
    console.log('   ⏳ This may take 5-10 seconds...');

    const response = await axios.post(
      `${FUNCTIONS_BASE_URL}/api/ClaudeLlmClient`,
      {
        prompt: {
    ...
    console.log('');
    console.log('✅ Haiku received from Azure!');
    console.log('');
    ...
    console.log('✅ Azure deployment test passed!');
    ...
    if (error.response) {
      ...
      // Check for missing Azure environment variables
      if (error.response.status === 401 && error.response.data?.message?.includes('ANTHROPIC_API_KEY')) {
        console.error('');
        console.error('💡 Azure Configuration Needed:');
        console.error('   The Azure Function App needs environment variables configured.');
        console.error('   Run these commands:');
        console.error('');
        console.error('   az functionapp config appsettings set \\');
        console.error('     --name jack-mcp-azure-ai-function \\');
        console.error('     --resource-group <your-resource-group> \\');
        console.error('     --settings "ANTHROPIC_API_KEY=<your-key>"');
        console.error('');
        console.error('   Or set via Azure Portal:');
        console.error('   Settings → Environment variables → Add ANTHROPIC_API_KEY');
      }
    }
    process.exit(1);
  }
