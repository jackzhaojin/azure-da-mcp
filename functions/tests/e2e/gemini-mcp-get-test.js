#!/usr/bin/env node
/**
 * E2E Test for GeminiLlmClient with MCP Tool Call
 * Tests Gemini with get_dalive_content MCP tool call + haiku generation
 *
 * Run with: node tests/e2e/gemini-mcp-get-test.js
 */
...
console.log('🚀 Testing GeminiLlmClient with MCP Tool Call (get_dalive_content)');
...
async function testGeminiMcpGet() {
  try {
    console.log('📝 Requesting Gemini to use MCP tool + write haiku...');
    console.log('   ⏳ This may take 10-20 seconds (includes MCP tool call)...');

    const response = await axios.post(
      `${FUNCTIONS_BASE_URL}/api/GeminiLlmClient`,
      {
        prompt: {
        ...
        // Model will use default from GEMINI_MODEL env var
      },
      {
        headers: {
