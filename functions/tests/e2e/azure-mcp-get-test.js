#!/usr/bin/env node
/**
 * E2E Test for AzureAIFoundryLlmClient with MCP Tool Call
 * Tests Azure AI Foundry with get_dalive_content MCP tool call + haiku generation
 *
 * Run with: node tests/e2e/azure-mcp-get-test.js
 */
...
console.log('🚀 Testing AzureAIFoundryLlmClient with MCP Tool Call (get_dalive_content)');
...
async function testAzureMcpGet() {
  try {
    console.log('📝 Requesting Azure AI Foundry to use MCP tool + write haiku...');
    console.log('   ⏳ This may take 10-20 seconds (includes MCP tool call)...');

    const response = await axios.post(
      `${FUNCTIONS_BASE_URL}/api/AzureAIFoundryLlmClient`,
      {
        prompt: {
        ...
        // Model will use default from AZURE_MODEL env var
      },
      {
        headers: {
    ...
    if (response.data.mcpToolCalls && response.data.mcpToolCalls.length > 0) {
      ...
      response.data.mcpToolCalls.forEach((call, index) => {
        ...
        if (call.result && call.result.htmlContent) {
          console.log(`      Result: HTML content (${call.result.htmlContent.length} chars)`);
        }
      });
    }
