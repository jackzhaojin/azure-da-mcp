#!/usr/bin/env node
/**
 * E2E Test for ClaudeAgentSdk with MCP Streamable endpoint
 * Tests Claude Agent SDK connecting to the mcp-streamable endpoint
 *
 * Run with: node tests/e2e/claude-agent-sdk-mcp-streamable-get.test.js
 */
...
console.log('🚀 Testing ClaudeAgentSdk with MCP Streamable Endpoint');
console.log(`   Server: ${FUNCTIONS_BASE_URL}`);
console.log(`   MCP Server: ${FUNCTIONS_BASE_URL}/api/mcp-streamable`);
console.log(`   Test Path: ${TEST_PATH}`);
console.log(`   Date: ${dateStr}`);
console.log(`   Time: ${timeStr}`);
console.log('');

async function testClaudeAgentSdkMcpStreamableGet() {
  try {
    console.log('📝 Requesting Claude Agent SDK to use MCP Streamable + write haiku...');
    ...
    const response = await axios.post(
      `${FUNCTIONS_BASE_URL}/api/ClaudeAgentSdk`,
      {
        ...
        mcpConfig: {
          serverUrl: 'http://localhost:7071/api/mcp-streamable',  // Using mcp-streamable endpoint
    ...
    if (!response.data.mcpToolCalls || response.data.mcpToolCalls.length === 0) {
      ...
      if (!getContentCall) {
        ...
        console.log('✅ MCP tool get_dalive_content was successfully called via mcp-streamable endpoint!');
      }
    }
...
testClaudeAgentSdkMcpStreamableGet();
