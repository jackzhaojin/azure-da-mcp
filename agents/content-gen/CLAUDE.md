...
- `src/generator.ts` — the deterministic template tier + the shapes. `generateBrief()` (coherent topic-substituted prose, NOT lorem); `synthesizeSource()` renders a `Brief` into standalone legacy HTML (`clean | dated | messy`, multi-paragraph + byline + dek + **typed feature blocks**) plus a `groundTruth` object eval scores fidelity against. The `Brief`/`SyntheticSource` shapes ARE the contract artifacts — agentic + template both return them. Also holds **`TOPIC_LANES` + `ideateTopic()`** (the deterministic lane picker) and, since v2.4 (2026-06-27), the **`wilderness-journal` lane** + a **real Unsplash photo pool (`WILDERNESS_IMAGE_IDS`/`sceneryImageUrl`) that replaced the old picsum placeholders** — so a generated article carries a real, scenic hero image (section 0 = w=1600). "The header image is the whole vibe."
...
- **`DEFAULT_LANE` stays `postal-logistics`, but the demo runs `wilderness-journal`** — don't be fooled. `ideateTopic`'s intrinsic default is still `postal-logistics` because `content-gen.e2e` pins it (no-lane + unknown-lane → that lane). The live lane comes from the **coordinator's `site-profiles.ts`** (`adapt-to-2026-demo` → `wilderness-journal`), which the coordinator passes to `content.ideate`. So "what does the daily loop write about?" is answered in the COORDINATOR's site profile, not here. Likewise the Wilderness-Journal **voice** arrives as the `siteBrief` the coordinator threads in — content-gen has no hardcoded site identity.
