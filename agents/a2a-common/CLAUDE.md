...
- `src/dalive.ts` — **da.live client (v2.9)**: stateless JSON-RPC `tools/call` against the deployed `functions/` MCP (`DALIVE_MCP_URL`; the server self-authenticates via S2S, `DALIVE_BEARER_TOKEN` optionally attributes writes). `getDaliveContent(path)` / `saveDaliveContent(path, html)` / `DaliveError` (`.notFound`). Deliberately NO default URL — memory is opt-in per process.
- `src/memory.ts` — **the agent-memory page (v2.9)**: `memorySourcePath(owner, site, "ai-content/memory")`, `readMemory(path)` (→ `{ html, text, chars, entries, missing }`), `htmlToMemoryText` (da.live HTML → markdown-ish prompt text; links kept, sections → `---`), `memoryPromptExcerpt` (head + newest entries under a char cap), `renderMemoryEntry` / `appendMemoryEntry` (one dated `<div>` section appended inside `<main>`, never rewriting existing bytes), `appendMemoryToDalive` (re-reads right before saving). Used by the migration agent (read) and eval-service `eval.reflect` (write).
...
- **Memory/da.live access is gated on an explicit `DALIVE_MCP_URL`** (`daliveMcpUrl()` returns undefined without it → `readMemory` returns null, writes throw). The fast e2e tier strips `DALIVE_MCP_URL` + `DALIVE_BEARER_TOKEN` from spawned agents (`e2e/helpers/mesh.ts`) so tests never touch the real memory page; the migration backends' own MCP wiring still has its historical default constant — that's a different code path.
- **Memory is append-only by construction.** `appendMemoryEntry` never rewrites what a human or an earlier run wrote; there is no "update" API on purpose. **Two-tier placement (v2.9.1):** when the page has a section whose `<h2>` contains "Episodic", the entry is inserted INSIDE that section (as an `<h3>` block at its end, found with a depth-aware `<div>` walk of `<main>`); a page without that heading gets the v2.9.0 behavior (a new section before `</main>`). The "Approved memory" tier above it is human/compaction-owned and never touched by code.
