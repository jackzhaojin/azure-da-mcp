...
- **Memory is append-only by construction.** `appendMemoryEntry` inserts before `</main>` and never rewrites what a human or an earlier run wrote; there is no "update" API on purpose.
