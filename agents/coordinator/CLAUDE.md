...
- The coordinator dashboard (trigger, live runs, run detail) — `app/` + `components/` + `lib/`. Trigger lanes: full-loop / generate+migrate / **migrate-a-real-page** (multi-URL textarea → one branch per URL, target-folder override, per-run Kimi model, evaluate-after toggle → goal `auto`) / evaluate. Stage-demo UX plan: [`ai-docs/2026-08-02-migrate-website-demo/`](../../ai-docs/2026-08-02-migrate-website-demo/UX-PLAN.md).

## Key files
- `src/site-profiles.ts` — **per-target-site profiles** keyed by `payload.site`: the editorial lane + voice threaded to content-gen, and the target folder + reference corpus (`blockLibraryUrl`/`neighborPageUrl`) + prompt `pattern` threaded to migration. `adapt-to-2026-demo` → Wilderness Journal (lane `wilderness-journal`, folder `ai-articles`, reference `/ai-content/**`, pattern `article`); unprofiled sites get an empty profile (generic behavior preserved). This is how the same generic mesh produces site-appropriate content. A per-run `payload.folder` beats the profile's `contentFolder` (the demo convention: real-source migrations land in a dated folder like `travel-journal-2026-08-02`).
...
- **`goal: auto` uses the deterministic state table, NOT an LLM** (planner is M3). Inference: `alreadyMigratedUrl` ⇒ `evaluate`; `sourceLocation` **or `sources[]`** ⇒ `migrate,evaluate`; `topic` ⇒ full loop; none ⇒ throws. Don't expect semantic planning yet.
...
- **Fan-out shape differs by route**: evaluate-only fans out per target × `fanOut`; a generate-less migrate route with `sources[]` fans out per source URL × `fanOut` ("migrate the website" — each branch gets its own sourceUrl and a slug derived from ITS url, explicit `pageSlug` is single-source only); other pipeline routes fan out `fanOut` branches of one config. Concurrency capped by `COORD_FANOUT_CONCURRENCY` (default 2).
