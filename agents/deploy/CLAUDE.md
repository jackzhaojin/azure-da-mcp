...
- **Cold starts**: ~5-30s per container (bigger images slower). First request after idle pays it; sleepAfter: 15m across all four (cost: scale-to-zero; any dashboard visit wakes the coordinator, mesh calls wake the rest).
