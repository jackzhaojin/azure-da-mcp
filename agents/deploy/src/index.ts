...
export class CoordinatorContainer extends Container<Env> {
  ...
  constructor(ctx: DurableObjectState, env: Env) {
    ...
    this.envVars = {
      ...
      // memory entries link back to the run on the dashboard host
      DASHBOARD_PUBLIC_BASE: HOSTS.dash,
...
export class EvalContainer extends Container<Env> {
  ...
  constructor(ctx: DurableObjectState, env: Env) {
    ...
    this.envVars = {
      ...
      EVAL_CONCURRENCY: "1",
      BROWSER_PERMITS: "2",
      // eval.reflect (agent-memory write-back, v2.9) appends to the site's memory
      // page through the same deployed da.live MCP the migration agent authors with
...
export class ContentGenContainer extends Container<Env> {
  ...
  constructor(ctx: DurableObjectState, env: Env) {
    ...
    this.envVars = {
      ...
      // Agentic content backend (writes the real source page). Same OAuth recipe
      // as eval: the token + the .claude.json oauthAccount stanza (generated by
      // content-gen-entrypoint.sh). Unset → deterministic template tier.
      CLAUDE_CODE_OAUTH_TOKEN: env.CLAUDE_CODE_OAUTH_TOKEN,
      CLAUDE_ACCOUNT_UUID: env.CLAUDE_ACCOUNT_UUID,
      CLAUDE_EMAIL: env.CLAUDE_EMAIL,
      CLAUDE_ORG_UUID: env.CLAUDE_ORG_UUID,
