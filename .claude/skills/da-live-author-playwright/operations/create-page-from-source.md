...
- **Filling in image data you don't have.** If the source has images with working external URLs (most common when migrating a webpage), use those URLs **directly** in the da.live HTML — EDS auto-ingests external images into the local media bus on preview-publish. There is no separate upload step. See `references/eds-html-structure.md` "Images" for the two valid `src` shapes. Only fall back to a placeholder (`/media/placeholder.jpg`) plus a gap note when no URL exists at all (e.g., images embedded in a PDF you can't extract). Don't invent URLs, and don't burn cycles trying to download-and-rehost reachable external images.
